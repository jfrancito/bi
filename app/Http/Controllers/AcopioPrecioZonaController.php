...
class AcopioPrecioZonaController extends Controller
{
    use UserTraits;
    public function actionListarPrecioZona($idopcion)
    {
        /******************* validar url **********************/
        ...
        /******************************************************/
        View::share('titulo', 'Precio por Zona');
    ...
    public function actionAjaxListarPrecioZona(Request $request)
    {
        ...
        $query = DB::table('acopio_arroz_raw')
            ...
            ->groupBy('anio_compra', 'centro_acopio_nombre', 'zona_comercial')
            ->orderBy('anio_compra', 'desc')
            ->orderBy('centro_acopio_nombre')
            ->orderBy('zona_comercial');

        $data = $query->get();
        return response()->json($data);
    }

    public function actionExportarDetallePrecioZona(Request $request)
    {
        $anio = $request->input('anio');
        $mes = $request->input('mes');
        ...
        $query = DB::table('acopio_arroz_raw')
            ...
            ->when($zona, function ($q) use ($zona) {
                return $q->where('zona_comercial', $zona);
            });
        ...
        $filename = "detalle_precio_zona_" . date('Ymd_His') . ".csv";
        $headers = array(
    ...
    public function actionAjaxComparativoPreciosMensual(Request $request)
    {
        $anio = $request->input('anio');
        $centro = $request->input('centro');
